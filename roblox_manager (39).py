...
DEFAULT_SERVERS = {
    ...
    "farm": {
        "name": "Farm Server",
        "place_id": 133322550157181,
        "link_code": "78782432814231717861076663443421",
    },
}
...
class APIHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if len(parts) >= 2 and parts[0] == "restart":
            ...
            # ── KILL ALL ROBLOX PROCESSES IMMEDIATELY ──
            ...
            def delayed_relaunch():
                ...
                for acc_name in relaunch_accounts:
                    ...
                    print(f"[RESTART] Relaunched {acc_name} → {server_key}: {result}")
                    ...
                    for acc_name in missing:
                        ...
                        if inst and inst.get("pid"):
                            try:
                                ...
                                pass
                        result = manager.launch_instance(acc_name, server_key)
                        print(f"[VERIFY] Re-relaunched {acc_name} → {server_key}: {result}")
                        time.sleep(2)
                else:
